...
export default function PostCarouselSection({ category, title }: PostCarouselSectionProps) {
  ...
  return (
    <div className="mb-16 px-4 bg-primary">
      <h3 className="text-3xl font-semibold text-center text-text mb-6 capitalize" data-aos="fade-up">
        {title}
      </h3>
            ...
            {posts.map((post, idx) => (
              ...
              >
                <div className="relative rounded-xl overflow-hidden h-[320px] shadow-lg transition-transform duration-300 ease-out hover:-translate-y-2 border border-accent2">
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-4 bg-secondary bg-opacity-90 flex justify-between items-center">
                    <Link
                      to={`/posts/${post.slug}`}
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm text-text line-clamp-1">{post.title}</span>
